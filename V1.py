...
class StressTestRunner:
    ...
    def generate_unique_transaction(self, iteration: int, num_unique_tokens: int = 50) -> str:
        """
        Generate realistic financial transaction descriptions.
        Each transaction has unique identifiers to simulate vocabulary growth.
        """
        transaction_types = [
    ...
    def detect_degradation(self, batch_num: int, batch_time: float):
        ...
        if len(self.batch_times) >= rolling_window:
            ...
            if degradation_factor >= 3.0:
                ...
                print(f"\n⚠️  CRITICAL DEGRADATION at batch {batch_num}: {batch_time:.2f}s ({degradation_factor:.1f}x baseline)")
                ...
                self.degradation_events.append(event)
                print(f"\n⚠️  WARNING: Degradation at batch {batch_num}: {batch_time:.2f}s ({degradation_factor:.1f}x baseline)")
    ...
    def run_stress_test(self, output_folder: str, checkpoint_every: int = 100):
        ...
        try:
            while self.running:
                ...
                # Generate unique transactions (NOT timed)
                descriptions = []
                memos = []
                ...
                # Send request and time ONLY the API call
                ...
                print(f"Batch {batch_num}: {api_time:.2f}s | Status: {status_code} | Total TXNs: {batch_num * self.batch_size:,}", end='\r')
                
                if batch_num % checkpoint_every == 0:
                    self.save_checkpoint(batch_num, output_folder)
                    print(f"\nCheckpoint saved at batch {batch_num}")
