...
# This is your robust "rolling 30" setting
...
class StressTestInvestigator:
    ...
    and intelligently stop when the service is confirmed to be 100% unresponsive.
    """
    ...
    # ---
    # This is your perfect transaction generator. It is unchanged.
    # ---
    ...
    # ---
    # This is the "Investigator" send function
    # This version includes the CRITICAL BUG FIX
    # ---
    def send_batch_request(self, descriptions: List[str], memos: List[str]) -> dict:
        ...
        try:
            ...
            try:
                # *** THIS IS THE FIX ***
                # We ALWAYS try to parse JSON, for both 200 OK and 500 Error
    ...
    # ---
    # This function reads from our "Evidence Locker"
    # ---
    def detect_degradation(self, batch_num: int, batch_time: float):
        ...
        if self.baseline_avg is None:
            return # Can't detect degradation without a baseline
    ...
    # ---
    # This is your "Investigation Mode" sampler, saving 200s and 500s
    # ---
    def log_investigation_samples(self, triggering_batch: int, window_start: int, error_rate: float):
        ...
        """
        # Find up to N successful and N failed samples from the window
        ...
        if len(success_samples) > 0 and len(failure_samples) > 0:
            # Standard case: log up to 3 of each
            num_samples_to_log = min(len(success_samples), len(failure_samples), INVESTIGATION_SAMPLES)
        else:
            # Edge case: log 3 samples of whatever we have (e.g., all failures)
            num_samples_to_log = INVESTIGATION_SAMPLES
        ...
        try:
            ...
            print(f"\n--- ERROR writing to investigation_log.csv: {e} ---")


    # ---
    # The main loop with the single, robust 100% Stop Condition
    # ---
        ...
        try:
            while self.running:
                ...
                if result_dict['status_code'] == 200:
                    ...
                    is_first_failure = len([r for r in self.results_per_batch if r['status_code'] != 200]) == 1
                    if is_first_failure:
                         # This print is essential, as it's the first sign of trouble
                         print(f"\n❌ FIRST FAILURE at batch {batch_num}: Status {result_dict['status_code']}, Error: {result_dict['response_body']}")
                    ...
                    for threshold in INVESTIGATION_THRESHOLDS:
                        if error_rate >= threshold and threshold not in self.investigation_thresholds_hit:
                            ...
                            if threshold == STOP_FAILURE_RATE:
                                ...
                                self.running = False
                                # Break inner loop to stop immediately
                                break 
    ...
    def save_final_metrics(self, stop_reason: str):
        ...
        # The full response bodies are in the `investigation_log.csv`
        metrics_for_json = []
        for r in self.results_per_batch:
            error_msg = None
            if r['status_code'] != 200:
                if isinstance(r['response_body'], dict):
                    error_msg = r['response_body'].get('error')
                else:
                    error_msg = str(r['response_body'])
        ...
        final_data = {
            ...
            'failure_milestones': {
                ...
                'total_transactions_at_first_error': (first_error_result['batch_num'] * BATCH_SIZE) if first_error_result else None,
...
def main():
    print("="*80)
    print("API STRESS TEST (INVESTIGATOR SCRIPT v5 - Final)")
    print("="*80)
    print(f"\nAPI URL: {API_URL}")
    print(f"Batch Size: {BATCH_SIZE}")
    print(f"Output Folder: {OUTPUT_FOLDER}")
    print(f"Rolling Window: {ROLLING_WINDOW_SIZE} batches")
